...
def _generate_polygons(world, args=None):
    ...
    surf = world.GetFlag('GroundLevel')
    rock = world.GetFlag('RockLevel')
    ...
    polys.append(("+ymin", 0))
    polys.append(("+ymax", h))
    polys.append(("+name", world.GetFlag("Title")))
    ...
    poly_append(world, polys, 'Jungle',
                lambda t: t.Wall == IDs.Wall.MudUnsafe or \
                          t.Wall == IDs.Wall.JungleUnsafe,
    ...
    if world.GetWallCount(IDs.Wall.LihzahrdBrickUnsafe) > 0:
        poly_append(world, polys, 'Temple',
                    ...
                    simplify=True, shortcircuit=True)
    ...
    polys.append(('OceanL', _box2(left, 380, top, surf+10)))
    polys.append(('OceanR', _box2(right-308, right, top, surf+10)))
...
def _do_find_arg(p, args, w, out, tile2str, *vargs, **kwargs):
    ...
    if args.csv and args.detailed_csv:
        writer = csv.writer(out)
        m = MapFile.Map()
        m.FromWorld(w)
        lookupArgs = kwargs.get("argsTileToLookup", dict())
        writer.writerow(['Attribute', 'Value', 'Tile', 'Red', 'Green',
                         'Blue', 'x', 'y'])
        writer.writerow(['Width', w.Width(), -1, '', 0, 0, 0, 0, 0])
        writer.writerow(['Height', w.Height(), -1, '', 0, 0, 0, 0, 0])
        for name, depth in w.GetLevels().items():
            writer.writerow([name, depth, -1, '', 0, 0, 0, 0, 0])
        for t, c, r in matches:
            table, lookup, option = m.TileToLookup(t, c, r, **lookupArgs)
            color = m.DoColorLookup(table, lookup, option)
            if color is None:
                color = (0, 0, 0) # black
            color = list(color[0:3])
            writer.writerow(["", "", t.Type, t.Name] + color + [c, r])
    elif args.csv:
        writer = csv.writer(out)
...
"""
    ...
    if args.list:
        ...
        for fname, world, fpath in World.World.ListWorlds():
            ...
            attribs = "Unknown"
            if (width, height) == World.SIZE_SMALL:
                attribs = "Small"
            elif (width, height) == World.SIZE_MEDIUM:
                attribs = "Medium"
            elif (width, height) == World.SIZE_LARGE:
                attribs = "Large"
            if world.GetFlag('IsCrimson'):
                attribs += ", Crimson"
            else:
                attribs += ", Corruption"
            if world.GetFlag('ExpertMode'):
                attribs += ", Expert"
            wname = world.GetFlag('Title')
    ...
    if args.tile_table:
        ...
        var = "TILES_%s = [" % (_make_token_from(w.GetFlag("Title")),)
        if args.tile_table_expr:
            var = "["
        out.write(var)
        out.write("\n")
        for row in range(w.GetFlag("TilesHigh")):
            # w.EachTile() won't work here because of how this is laid out
            tileRow = []
            for col in range(w.GetFlag("TilesWide")):
